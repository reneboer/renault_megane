...
class RenaultDataUpdateCoordinator(DataUpdateCoordinator[T]):
    ...
    async def _async_update_data(self) -> T:
        ...
        except ChargeModeInProgressException as err:
            # This is a temporary error when performing charge mode change requests too quickly.
            raise UpdateFailed(f"A remote charge mode change request is in progres. Please wait...: {err}") from err

        except PrivacyModeOnException as err:
            # This is a permanent error until user allowed remote data access again in car.
            if not self._has_already_worked:
                self.update_interval = None
                self.access_denied = True
